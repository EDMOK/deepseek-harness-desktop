...
<a href="https://github.com/EDMOK/deepseek-harness-desktop/releases/latest"><strong>下载最新 Windows ZIP</strong></a>
...
| Host API、RPC、HTTP 与事件流 | [`packages/host`](packages/host)、[`packages/api`](packages/api)、[`docs/api-gateway.md`](docs/api-gateway.md) |
| CLI 分发与 Profile | [`apps/cli`](apps/cli)、[`packages/boot/app-boot`](packages/boot/app-boot) |
| 内置 bundle 与 Profile 组合 | [`packages/bundle`](packages/bundle)、[`docs/architecture.md`](docs/architecture.md) |
...
| 扩展示例 | [`docs/cookbook`](docs/cookbook)、[`examples`](examples) |

修改 `packages/` 前请先阅读 [`docs/architecture.md`](docs/architecture.md)。该文档说明插件树、Profile 层、能力 seam、会话日志和扩展点。
...
源码 Web UI 由 `dsh web` Profile 在本地提供。开发 Client 插件时，请按照 [`docs/api-gateway.md`](docs/api-gateway.md) 中的说明，同时启动源码 launcher 和 Client watcher。
...
请针对改动选择最小的相关检查。完整 gates 和平台策略见 [`CLAUDE.md`](CLAUDE.md) 与 [`docs/testing.md`](docs/testing.md)。
...
- [能力 seam](docs/capability-seams.md)

社区插件可以添加 [`dsh-plugin`](https://github.com/topics/dsh-plugin) 话题，便于被发现。

## 社区与支持

- 在 [GitHub Discussions](https://github.com/EDMOK/deepseek-harness-desktop/discussions) 讨论产品行为或报告 bug。
