...
<a href="https://github.com/EDMOK/deepseek-harness-desktop/releases/latest"><strong>Download the latest Windows ZIP</strong></a>
...
| Host API, RPC, HTTP, and event streams | [`packages/host`](packages/host), [`packages/api`](packages/api), [`docs/api-gateway.md`](docs/api-gateway.md) |
| CLI dispatch and profiles | [`apps/cli`](apps/cli), [`packages/boot/app-boot`](packages/boot/app-boot) |
| Built-in bundles and profile composition | [`packages/bundle`](packages/bundle), [`docs/architecture.md`](docs/architecture.md) |
...
| Extension examples | [`docs/cookbook`](docs/cookbook), [`examples`](examples) |

Read [`docs/architecture.md`](docs/architecture.md) before changing packages. It explains the plugin tree, profile layers, capability seams, session log, and extension points.
...
The source Web UI is served locally by the `dsh web` profile. For client-plugin development, use the source launcher together with the client watcher described in [`docs/api-gateway.md`](docs/api-gateway.md).
...
Use the smallest relevant check for a change. The repository's full gates and platform policy are documented in [`CLAUDE.md`](CLAUDE.md) and [`docs/testing.md`](docs/testing.md).
...
- Report bugs and discuss product behavior in [GitHub Discussions](https://github.com/EDMOK/deepseek-harness-desktop/discussions).
